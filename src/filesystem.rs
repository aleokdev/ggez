...
use crate::{
    conf,
    vfs::{self, OverlayFS, VfsError, VFS},
...
impl Filesystem {
    ...
    pub fn new(
        ...
        let project_dirs =
            ProjectDirs::from("", author, id).ok_or(FilesystemError::NoValidHomePath)?;
    ...
    /// in read-only mode.
    pub fn open<P: AsRef<path::Path>>(&self, path: P) -> FilesystemResult<File> {
        self.vfs()
            .open(path.as_ref())
    ...
    pub fn create<P: AsRef<path::Path>>(&self, path: P) -> FilesystemResult<File> {
        self.vfs()
            .create(path.as_ref())
            .map(|f| File::VfsFile(f))
            .map_err(FilesystemError::from)
    ...
    pub fn create_dir<P: AsRef<path::Path>>(&self, path: P) -> FilesystemResult {
        self.vfs()
            .mkdir(path.as_ref())
            .map_err(FilesystemError::from)
    }
    ...
    pub fn delete<P: AsRef<path::Path>>(&self, path: P) -> FilesystemResult {
        self.vfs().rm(path.as_ref()).map_err(FilesystemError::from)
    }

    /// Deletes the specified directory in the user dir,
    /// and all its contents!
    pub fn delete_dir<P: AsRef<path::Path>>(&self, path: P) -> FilesystemResult {
        self.vfs()
            .rmrf(path.as_ref())
            .map_err(FilesystemError::from)
    }
...
impl From<VfsError> for FilesystemError {
    fn from(s: VfsError) -> Self {
        FilesystemError::VfsError(s)
    }
}
...
impl std::fmt::Display for FilesystemError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FilesystemError::NoValidHomePath => {
                f.write_str("no valid home directory path could be retrieved")
            }
            FilesystemError::VfsError(err) => err.fmt(f),
        }
    }
}
...
impl std::error::Error for FilesystemError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            FilesystemError::VfsError(err) => Some(err),
            _ => None,
        }
    }
}
...
pub fn open<P: AsRef<path::Path>>(ctx: &Context, path: P) -> GameResult<File> {
    ctx.fs.open(path).map_err(GameError::from)
}
...
pub fn open_options<P: AsRef<path::Path>>(
    ...
    ctx.fs.open_options(path, options).map_err(GameError::from)
}
...
pub fn create<P: AsRef<path::Path>>(ctx: &Context, path: P) -> GameResult<File> {
    ctx.fs.create(path).map_err(GameError::from)
}
...
pub fn delete_dir<P: AsRef<path::Path>>(ctx: &Context, path: P) -> GameResult {
    ctx.fs.delete_dir(path.as_ref()).map_err(GameError::from)
}
    ...
    fn headless_test_file_not_found() {
        let fs = dummy_fs_for_tests();
        {
            ...
            match fs.open(rel_file) {
                Err(FilesystemError::VfsError(VfsError::ResourceNotFound { .. })) => (),
